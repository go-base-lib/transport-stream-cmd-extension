...
type CmdHandler func(stream *transportstream.Stream) (ExchangeData, error)

var cmdMap = map[CmdName]CmdHandler{}

func CmdRoute(stream *transportstream.Stream) error {
	sendEndOk := false
	defer func() {
		if sendEndOk {
			return
	...
	if nextData, err := cmdHandle(stream); err != nil {
